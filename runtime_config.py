...
DEFAULT_SUPPORTED_CHANNELS = [6]  # range(1,14)
...
@dataclass
class RuntimeConfig:
    ap_wlan_interface: str = "wlxc83a35c2e034"
    monitor_wlan_interface: str = "wlxc83a35c2fcb0"
    gateway_interface: str = "ens33"
    attacked_ap: AccessPointInfo = dataclasses.field(default_factory=lambda:  AccessPointInfo(**_org['attacked_ap']))
    attacked_endpoint: str = '28:ea:2d:c6:27:f3'
    ...
    def validate(self):
        for f, v in self.__dict__.items():
            if not v:
                raise RuntimeError(
                    f'{f} is not set.\n'
                    f'Please make sure to configure all interfaces, select a target AP and endpoint.')

        if self.monitor_wlan_interface == self.ap_wlan_interface:
            raise RuntimeError("AP and Monitor ifaces are the same...")

        if self.monitor_wlan_interface == self.gateway_interface:
            raise RuntimeError("Gateway and Monitor ifaces are the same...")
